 ...
 * 저는 함수 API 가 GetXXX 라고 만들어질 경우 해당 함수가 의미있는 함수로 분기되지 않을 경우 해당 이름을 그대로 활용하여
 * 처음 시작한 부분과 거리감을 두지 않기 위해 해당 부분에 대하여 만든 부분입니다.
 * ex) API 이름이 GetProductList -> DTO 이름, Service 이름, 등 비슷하거나 동일한 의미가 사용되는 부분이라면 하나로 통일하고 있습니다.
